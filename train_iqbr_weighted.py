...
dataset = PleiadesDataset(root=r"D:/deep_learning/samples/jeux_separes/train/iqbr_cl_covabar_10mdist_obcfiltered3_rgb/", views=1, transform=base_transforms, indices = None)
...
test_dataset = PleiadesDataset(root=r"D:/deep_learning/samples/jeux_separes/test/iqbr_cl_covabar_10mdist_obcfiltered3_rgb/", views=views, transform=test_transforms, indices = test_sampler.indices, expand=True)
...
learning_rate = 8e-5 # à ajuster au besoin!
...
for epoch in range(epochs):
    ...
    # valid_accuracy = float((class_report.split('accuracy')[1][27:31]))
    valid_accuracy = round(len(((y_pred_mode == y_true).nonzero())[0]) / len((y_pred_np[0])), 4)
    valid_accuracies.append(valid_accuracy)
    print('valid mode accuracy: ' + str(valid_accuracy))
...
comment =  f'L4, centercrop, {permutations} permutations, (train val = 85-15), poids single img, classificateur 512-do-512, MEAN POOL, classes COV, CJ 0.2, AUGMENT + RESHUFFLE T_DS et VAL_DS, views et segments rives isoles par dataset (une fois chaque image),random sampler, {views} views, seed = {seed}'

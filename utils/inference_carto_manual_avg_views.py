...
for c in checkpoints:
    ...
    for i in range(10):
        ...
        for minibatch in test_loader:
            ...
            for p, s in zip(avg_prediction, segment_id):
...
segment_predictions =  [[segment_ids[x]] + [str(int(rounded_pred[x]))] for x in range(len(segment_ids))]

with open('predictions/' + checkpoints[0] +'manual_avg1v' + '.csv', 'w', newline='') as f:
    write = csv.writer(f)

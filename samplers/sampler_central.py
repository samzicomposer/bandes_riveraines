...
"""
Ce script crée des points de chaque côté des cours d'eau du secteur central. Deux shp distincts sont utilisés pour ce secteur, un pour chacune des rives.
"""
...
################################################################################
## path to the
shp_droite = r"I:/donnees/Covabar/nettoyage/IQBR_Droite_ls_utm.shp"
shp_gauche = r"I:/donnees/Covabar/nettoyage/IQBR_Gauche_ls_utm.shp"
...
output_paral = r"K:/deep_learning/sampling/centre/{}_parall_{}m".format(os.path.basename(shp_gauche[:-4]), distance_paral)
## output point fc name
output_pts = r"K:/deep_learning/sampling/centre/{}_2m_{}m_points".format(os.path.basename(shp_gauche[:-4]), distance)
...
for line1 in tqdm(lyr1):
    ...
    gps1 = line1.GetField('layer')[-3]
    fid1 = line1.GetField('fid')
    largeur1 = line1.GetField('Largeur_i')
    iqbr1 = line1.GetField('IQBR_i')

    for line2 in lyr2:
        station2 = line2.GetField('Station')
        gps2 = line2.GetField('layer')[-3]
        fid2 = line2.GetField('fid')
        largeur2 = line2.GetField('Largeur_i')
        iqbr2 = line2.GetField('IQBR_i')

        # if station1 == station2  and fid1 == fid2 and gps1 == gps2 and [fid1, station1, gps1] not in stations_numbers \
        #         and any( (z[0] <= largeur1  <= z[1]) and (z[0] <= largeur2  <= z[1]) for z in classes):
        # if station1 == '245' and station2 == '245':
        #     break
        ...
        # if station1 == station2 and fid1 == fid2 and gps1 == gps2 and [fid1, station1, gps1] not in stations_numbers:
            ...
            # if station1 == station2  and fid1 == fid2 and gps1 == gps2 and [fid1, station1, gps1] not in stations_numbers and \
        #         abs(largeur1 - largeur2) <= 1.0:
            pairlist.append([line1, line2])
            stations_numbers.append([fid1,station1,gps1])
...
for ln in tqdm(pairlist):
    ...
    try: # certaines valeurs de différence sont nulles en raison de l'absence d'OBC (très petits segments)
        ...
        if paral_right_1.distance(paral_left_1) < paral_right_2.distance(paral_left_1): # inversé pour images sentinel, on s'éloigne de la rive
            linestring_list.append({'geom': paral_right_1, 'Largeur': ln[0].GetField('Largeur_i'), 'IQBR': ln[0].GetField('IQBR_i'), 'serial': ln[0].GetField('serial'),'test_ds':ln[0].GetField('test_ds'),
                                    'iqbr_diff':abs(ln[0].GetField('IQBR_i') - ln[0].GetField('OBC_iqbr')), 'OBC_iqbr': ln[0].GetField('OBC_iqbr')})
            # linestring_list.append({'geom': paral_right_1, 'Classe': 1})
        else:
            linestring_list.append({'geom': paral_right_2, 'Largeur': ln[0].GetField('Largeur_i'), 'IQBR': ln[0].GetField('IQBR_i'), 'serial': ln[0].GetField('serial'),'test_ds':ln[0].GetField('test_ds'),
                                    'iqbr_diff':abs(ln[0].GetField('IQBR_i') - ln[0].GetField('OBC_iqbr')), 'OBC_iqbr': ln[0].GetField('OBC_iqbr')})
        ...
        if paral_left_1.distance(paral_right_1) < paral_left_2.distance(paral_right_1):  # inversé pour images sentinel, on s'éloigne de la rive
            linestring_list.append({'geom': paral_left_1, 'Largeur': ln[1].GetField('Largeur_i'), 'IQBR': ln[1].GetField('IQBR_i'), 'serial': ln[1].GetField('serial'),'test_ds':ln[1].GetField('test_ds'),
                                    'iqbr_diff':abs(ln[1].GetField('IQBR_i') - ln[1].GetField('OBC_iqbr')), 'OBC_iqbr': ln[1].GetField('OBC_iqbr')})
            # linestring_list.append({'geom': paral_right_1, 'Classe': 1})
        else:
            linestring_list.append({'geom': paral_left_2, 'Largeur': ln[1].GetField('Largeur_i'), 'IQBR': ln[1].GetField('IQBR_i'), 'serial': ln[1].GetField('serial'),'test_ds':ln[1].GetField('test_ds'),
                                    'iqbr_diff':abs(ln[1].GetField('IQBR_i') - ln[1].GetField('OBC_iqbr')), 'OBC_iqbr': ln[1].GetField('OBC_iqbr')})
